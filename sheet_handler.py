...
def get_skill(sheet_inc, skill_inc: str):
	...
	eloquence_found = False
	rogue_found = False
	mothfolk_found = False
	barbarian_found = False

	for line in class_area:
		while len(line) < 4:
			line.append("")

		if line[1] == "Eloquence" and not eloquence_found:
		...
		if line[0] == "Rogue":
			...
			for invocation in warlock_invocations:
				if invocation[0] == "Lifedrinker":
					followups.append(c.FollowupButton(bot.get_emoji(1237415705574506537), "cha[necrotic]", "roll", "lifedrinker"))
